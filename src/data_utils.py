"""Adapted from https://github.com/eurecom-asp/rawnet2-antispoofing/blob/main/data_utils_LA.py (MIT license)"""
...
class ASVDataset(Dataset):
    ...
    def __init__(
        ...
        if os.path.exists(self.cache_fname):
            ...
            data = []
            for meta in tqdm(self.files_meta, desc='Reading files', unit='file'):
                data.append(self.read_file(meta))
            self.data_x, self.data_y, self.data_sysid = map(list, zip(*data))
            
            if self.transform:
                print('Applying transformations...')
                transformed_data = []
                for x in tqdm(self.data_x, desc='Transforming', unit='sample'):
                    transformed_data.append(self.transform(x))
